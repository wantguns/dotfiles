...
in
{
  mkHost =
    ...
    let
      ...
      homeDirectory = if hostDarwin then "/Users/${username}" else "/home/${username}";
      ...
      systemBuilder =
        if hostDarwin then inputs.darwin.lib.darwinSystem else inputs.nixpkgs.lib.nixosSystem;
    in
    systemBuilder {
      inherit system;
      specialArgs = {
